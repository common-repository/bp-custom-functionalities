...
add_action( 'wp', 'bp_cfunc_guest_redirect', 1 );
...
function bp_cfunc_exclude_roles_members_dir( $ps = false, $object_name = false ) {
	$exc_roles = get_option( 'ps_exclude_roles', true );
	$users = array();

	if ( ! empty( $exc_roles ) && is_array( $exc_roles ) ) {
...
add_action( 'wp', 'bp_cfunc_restrict_levels' );
function bp_cfunc_restrict_levels() {
	...
	if ( ! empty( $exc_levels ) && ! empty( $membership_levels ) && is_array( $exc_levels ) ) {
		if ( is_buddypress() || bp_is_user() ) {
...
// Manage Members Count After Role Exclusion.
function bp_cfunc_hide_get_total_filter( $count ) {
	$exc_roles = get_option( 'ps_exclude_roles', true );
	$users = array();
	$roles = array();
...
add_filter( 'bp_get_total_member_count', 'bp_cfunc_hide_get_total_filter' );
